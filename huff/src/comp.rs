...
pub fn read_compress_write(src_path: PathBuf, dst_path: PathBuf, chunk_size: usize) -> Result<(), Box<dyn std::error::Error>>{
    // read from src file
...
/// 
/// Chunk size means how many bytes will be read from src file at one time
pub fn read_decompress_write(src_path: PathBuf, dst_path: PathBuf, chunk_size: usize) -> Result<(), Box<dyn std::error::Error>>{
    // read from src file
    let src = File::open(src_path)?;
    let mut src_bytes_left = src.metadata().unwrap().len() as usize;
    ...
    let bytes_read = reader.read(&mut buf).expect("HANDLE");
    assert_eq!(bytes_read, 5, "HANDLE");
    src_bytes_left -= 5;
    ...
    let bytes_read = reader.read(&mut buf).expect("HANDLE");
    assert_eq!(bytes_read, tree_len, "HANDLE");
    src_bytes_left -= tree_len;

    // read the HuffTree
    let tree = 
        huff_coding::prelude::HuffTree::<u8>::try_from_bin({
...
fn compress_to_writer<R: Read, W: Write + Seek>(
    ...
    while let Ok(_) = reader.read_exact(buf){
        ...
        writer.write_all(&comp_bytes).expect("HANDLE");
...
fn decompress_to_writer<R: Read, W: Write>(
    reader: &mut R, writer: &mut W, 
    reader_bytes_left: &mut usize, buf: &mut [u8],
    tree: HuffTree<u8>, padding_bits: u8){
    ...
    while let Ok(_) = reader.read_exact(buf){
        ...
        writer.write_all(&decomp_buf).expect("HANDLE");
        decomp_buf.clear();
        *reader_bytes_left -= buf.len();
    }
